...
@$sql = mysqli_query($con, "SELECT * from viagem where id_cliente = $id_cliente cliente  WHERE cpf = '$cpf'") or print mysql_error();
if(mysqli_num_rows($sql)<0) {
echo"cpf invalido";
}elseif ("hotel" == $estrela_1) {
@$valor_passagem=300;
  @$pacote= $valor_passagem + ($numero_acompanhantes * $valor_passagem)+($numeros_dias * 75);
  

   @$inserir = mysqli_query($con, "insert into viagem values('','$cpf',$numero_acompanhantes','$numeros_dias','$cidade_destino','$estrela_1','$data_saida','$data_chagada','$valor_passagem','$pacote')");  
